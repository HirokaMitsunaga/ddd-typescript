...
describe("GetBookApplicationService", () => {
  it("指定されたIDの書籍が存在する場合、DTOに詰め替えられ、取得できる", async () => {
    ...
    const createdBook = await bookTestDataCreator(bookRepository)({});
    const result = await getBookApplicationService.execute(
      createdBook.bookId.value
    );
    expect(result).toEqual(new BookDTO(createdBook));
  });
  it("指定されたIDの書籍が存在しない場合、nullが取得できる", async () => {
    ...
    const result = await getBookApplicationService.execute("9784167158057");
